...
class StartWindow:
    def __init__(self, root):
        ...
        self.bg_image = self.bg_image.resize((800, 600), Image.LANCZOS)  # Resize to fit the window
        ...
        # Create and place the Blackjack button
    ...
    def start_blackjack(self):
        self.root.destroy()  # Close the start window
        blackjack_root = tk.Tk()
        BlackjackGame(blackjack_root)
        blackjack_root.mainloop()
...
class BlackjackGame:
    def __init__(self, root):
        ...
        self.first_round = True  # New flag to track the first round

        self.create_widgets()

    # Modify the place_bet method
    def place_bet(self):
        try:
        ...
        if self.first_round:  # Only display the images in the first round
            # Destroy the image labels after placing the bet (for first round only)
            self.image1_label.destroy()
            self.image2_label.destroy()
            self.first_round = False  # Set the flag to False after the first round
    ...
    # Player Hand
        ...
        # Load the images for display (before cards)
    ...
    # Create the image labels and place them inside the dealer and player frames
        self.image1_label = tk.Label(self.dealer_frame, image=self.image1, bg="#35654d")
        self.image1_label.pack(side=tk.BOTTOM, pady=10)  # Adjust padding as needed
    
        self.image2_label = tk.Label(self.player_frame, image=self.image2, bg="#35654d")
        self.image2_label.pack(side=tk.BOTTOM, pady=10)  # Adjust padding as needed
    ...
    # Chip Buttons Frame
        ...
        for value in chip_values:
            ...
            chip_button.config(width=65, height=65)  # Adjust width and height as needed
    ...
    # Split Hand Frame (Initially hidden)
        self.split_label = tk.Label(self.root, text="Split Hand:", bg="#35654d", fg="white")
        self.split_frame = tk.Frame(self.root, bg="#35654d")
        self.split_label.pack_forget()  # Hide initially
        self.split_frame.pack_forget()  # Hide initially
